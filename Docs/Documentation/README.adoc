...
The main goal of these classes is to read information from an link:https://github.com/Sarah-Elhelw/teach_spreadsheets/blob/master/Docs/Documentation/Documents/AA_Saisie_des_voeux_2016-2017.ods[ods file] and create the correspanding CalcData object. The classes Course,Teacher and CoursePref that are created in these process are also returned in order to be used for other purposes (like storing in JSON format the list of courses available in the input file).