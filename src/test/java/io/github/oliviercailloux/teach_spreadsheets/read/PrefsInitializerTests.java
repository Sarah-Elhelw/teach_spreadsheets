	...
	void testCreatePrefsList() throws Exception {
		...
		try (InputStream stream = Files.newInputStream(infile)) {
			try (SpreadsheetDocument document = SpreadsheetDocument.loadDocument(stream)) {
				...
				// Checking the preferences for the course in cell P12 in the sheet DE1 : the aim here is to test that we can read course preferences in the semester 2.
				...
				// Checking the preferences for the course in cell B4 in the sheet DE2 : the aim here is to test that we can read course preferences in another sheet than DE1.
